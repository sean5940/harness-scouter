...
```
  ...
  2026-07-02 ~ 2026-08-04 · 세션 355개 · 이력 창 29개 · 커버리지 84%
  ────────────────────────────────────────────────────────────────────────────────
  ...
  검증력        ████████████████████░░░░  82  A   통상   74~89  최고  99
      커밋 전 검증 신선도   83   n=  392
      검증 공회전 없음      80   n= 1807
  완수력        █████████████████████░░░  88  A   통상   87~94  최고  98
      산출물 도달           94   n=  173  (표시)
      재작업 없음           88   n= 6034
  ...
  컨텍스트 효율 ██████████████████░░░░░░  73  B   통상   68~79  최고  86
      읽기 범위 규율        81   n= 1050
      읽은 것 기억하기      93   n=12906
      응답 간결성           54   n=22563
      컨텍스트 경량성       66   n=46206
  ────────────────────────────────────────────────────────────────────────────────
  종합 72.6 · B  (가장 가까운 등급 컷까지 5.4p)
...
**행동 파이프라인**은 트랜스크립트에서 사실만 뽑아 SQLite에 넣고, 점수는 매번 다시 계산합니다. 정의를 고칠 때 890MB를 재파싱하지 않으려는 구조입니다. `db.ts`에 점수가 없는 것이 그래서입니다.
...
가장 흔한 것이 `도구 이름에 묶임`입니다. 능력 층이 이름 대응은 풀었지만, **능력 자체가 없는 하네스**는 여전히 0점을 받습니다. 인덱스 도구가 없어서 0인 것과 있는데 안 써서 0인 것이 구별되지 않습니다.
