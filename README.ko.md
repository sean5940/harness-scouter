...
```
  HARNESS SCOUTER  전수 집계      Lv. 61  C
  2026-05-10 ~ 2026-08-14 · 세션 23개 · 이력 창 4개 · 커버리지 71%
  ────────────────────────────────────────────────────────────────────────────────
  탐색력        ████████░░░░░░░░░░░░░░░░  32  D   통상   38~50  최고  52
      파일 찾기 규율         0   n=   90
      내용 인덱스 우선      24   n=  372
      근거 확보율           71   n=  328
  검증력        ███████████░░░░░░░░░░░░░  45  D   통상   43~48  최고  49
      커밋 전 검증 신선도    3   n=   59
      검증 공회전 없음      87   n=  149
  완수력        █████████████████████░░░  86  A   통상   80~88  최고  91
      산출물 도달           73   n=   11  (표시)
      재작업 없음           86   n= 1192
  자율성        █████████████████████░░░  86  A   통상   81~95  최고  98
      사람 개입 없음        86   n= 5473
  규율          █████████████████░░░░░░░  70  B   통상   68~75  최고  77
      계측 채널 준수        56   n= 2130
      게이트 재발 없음      85   n=  167
  컨텍스트 효율 ██████████████████░░░░░░  74  B   통상   62~76  최고  86
      읽기 범위 규율        60   n=  259
      읽은 것 기억하기      91   n= 1066
      응답 간결성           76   n= 3196
      컨텍스트 경량성       68   n= 5988
  ────────────────────────────────────────────────────────────────────────────────
  종합 61.3 · C  (가장 가까운 등급 컷까지 0.7p)
  전수 집계라 등급을 절대 점수로 매겼습니다. 구간별 등급은 --all 없이 보세요.
```

숫자 자체보다 **어느 구성요소가 병목인지**가 쓸모입니다. 위 화면에서 `scouter guide --all` 은 검증력 45를 짚고 `커밋 전 검증 신선도 3 (n=59)` 하나를 지목합니다. 고치면 +48 입니다. 실제로 이 도구를 만들며 그렇게 썼습니다.

이 화면의 숫자는 2026-09-07 에 한 기계의 코퍼스 하나에서 나온 것입니다. 원본 출력은 `docs/measurements/` 에 있습니다. 당신의 트랜스크립트로 돌리면 다른 숫자가 나옵니다. 그게 요점이고, 동시에 이 숫자가 자신을 견줄 기준이 못 되는 이유입니다.
...
**행동 파이프라인**은 트랜스크립트에서 사실만 뽑아 SQLite에 넣고, 점수는 매번 다시 계산합니다. 정의를 고칠 때 166MB를 재파싱하지 않으려는 구조입니다. `db.ts`에 점수가 없는 것이 그래서입니다.

**하네스 구조 스캔**은 저장소에서 센서와 가이드의 목록을 읽습니다. 행동만 봐서는 "차단 0건"이 센서가 좋아서인지 없어서인지 갈리지 않기 때문입니다. 축 이름은 Birgitta Böckeler의 [harness engineering](https://martinfowler.com/articles/harness-engineering.html)에서 가져왔습니다.
...
```
~/.claude/projects/**/*.jsonl   읽기 전용 입력. 166MB
~/.harness-scouter/scouter.sqlite   사실 테이블. 27MB
...
| 테이블          | 담는 것                                           | 행 수(예) |
| --------------- | ------------------------------------------------- | --------- |
| `session`       | 세션 메타. 프로젝트·브랜치·모델·진입점            | 49        |
| `tool_call`     | 도구 호출. 이름·명령·파일 경로·차단 여부·에이전트 | 9,083     |
| `tool_result`   | 도구 결과. 읽은 줄 수·편집 종류·stdout 꼬리       | 9,083     |
| `usage`         | 응답당 토큰. 요청 단위로 중복 제거                | 7,209     |
| `session_event` | 중단·큐 개입·도구 거부                            | 339       |
| `artifact`      | 커밋·PR·커밋 해시                                 | 226       |
| `file_cursor`   | 파일별 mtime과 바이트 위치                        | 224       |

**축 점수를 저장하지 않는 것이 설계의 핵심입니다.** 지표 정의가 자주 바뀌는데, 정의를 고칠 때마다 166MB를 다시 파싱해야 하면 반복 주기가 무너집니다. 사실만 담아두고 축은 매번 계산합니다.
...
npm run scouter -- scan     # 166MB 전체 재파싱, 8초
```
...
| 시도      | 방법                                                | 기각 사유                                                                                      |
| --------- | --------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| 사람 라벨 | 세션마다 좋음·나쁨을 표시. 설계 단계에서 기각해 0건 | 세션마다 사람이 필요해 확장이 안 되고, 남의 하네스를 평가하려면 그 사람의 라벨을 받아야 합니다 |
...
테스트 396건입니다. 정의를 고칠 때는 회귀 테스트를 함께 고치세요 — 값이 바뀌는 종류라 정적 검사로는 안 잡힙니다.
