...
### "동작은 안 바뀐다"는 주장은 실행으로 대조한다

이 저장소 PR 은 "판정은 그대로다"·"게이트는 안 움직인다" 를 자주 주장한다. 코드를 읽어
확인하면 놓친다. 리팩터링이 난수 소비 순서를 한 칸 바꾸거나 누적 순서를 뒤집는 것은 읽어서
안 보이고, 테스트가 그 경로를 안 덮으면 아무것도 실패하지 않는다.

**두 빌드에 같은 입력을 넣어 출력을 바이트로 대조한다.** base 와 PR 을 각각 worktree 에
빌드하고, 결정론적 합성 코퍼스로 `runGate` 를 돌려 판정·인쇄값·상관을 전부 JSON 으로 찍은 뒤
`diff` 한다. 완전 일치가 아니면 주장이 틀린 것이다.

대조군은 **판정이 섞여 있어야** 한다. 전부 미달인 코퍼스에서의 일치는 아무것도 안 보장한다.
`packages/core/src/gate.ts` 의 축 정의를 채우는 합성 세션 수백 개로 통과·미달이 함께 나오는
코퍼스를 만든다(#4 검증에서 158 구간 · 통과 47 · 미달 13 을 썼다).

PR 을 보는 CI 가 없으므로 이 대조는 사람이 돌려야 한다.

## 7. 글로벌 컨벤션과의 차이

| 항목        | 글로벌                      | 이 프로젝트                                                                | 근거                        |
| ----------- | --------------------------- | -------------------------------------------------------------------------- | --------------------------- |
| 산출물 언어 | 한글                        | README는 영/한/일 3종 유지. `README.md` 가 영어본, 한국어는 `README.ko.md` | 공개 저장소                 |
