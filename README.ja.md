...
| 結果                   | 意味                                             | 次                                       |
| ---------------------- | ------------------------------------------------ | ---------------------------------------- |
| 上がる(下限超え)       | 区間スコアを揺らしたのは半分ごとに変わる作業構成 | 固定タスクセットに使う根拠ができます     |
| 変わらない             | 原因が作業構成ではない                           | タスクセットを作ってもこの軸は戻りません |
| 上がるが下限を超えない | ラベルに意味がなくても出る大きさの移動           | 読みません                               |

移動がどれだけ大きければ動いたことになるのかは、移動だけを見てもわかりません。そこで**プラセボ層**を一緒に回します。層の大きさはそのままにして、誰がどの層かだけを散らしたものなので、分割が受ける制約は実際の層化と同じでラベルだけが意味を失います。**意味のないラベルで測った移動がノイズの下限であり、実際の移動がそれを超えなければ証拠になりません。**

下限は見た目ほど低くありません。実際の移動がかろうじて超えるだけの位置に来ることがあり、プラセボがなければその値を「上がった」と読むことになります。
