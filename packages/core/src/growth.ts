...
export const COMPONENT_CRITERIA: Record<string, ComponentCriterion> = {
  ...
  "규칙 위반 시도 없음": {
    ...
    targetBasis:
      "차단이 없는 것이 목표라는 뜻에서는 ceiling 이지만 부호가 반대로 오염돼 있다. 훅을 하나도 안 깐 하네스가 자동으로 만점을 받는다. Fowler 도 같은 것을 미해결로 남겼다. 센서가 발화하지 않는 것이 품질이 좋아서인지 탐지가 부족해서인지. `scouter harness` 의 센서 수와 함께 읽어야 한다.",
    actions: [
